...
const features = [
  { icon: '🎤', title: 'Multi-Modal Input', desc: 'Voice, text, photo — citizens report issues naturally', href: '/features/multi-modal', color: '#1E40AF' },
  { icon: '🧠', title: 'AI Prioritization', desc: 'Smart scoring: Urgency × Impact × Recurrence × Sentiment', href: '/features/prioritization', color: '#B45309' },
  { icon: '📸', title: 'Proof-of-Work Verification', desc: 'GPS + timestamp + AI photo proof. No faking.', href: '/features/verification', color: '#047857' },
  { icon: '📊', title: 'Social Media Intelligence', desc: 'Real-time sentiment, early warnings, misinfo detection', href: '/features/social-media', color: '#6D28D9' },
  { icon: '📝', title: 'AI Communication Engine', desc: 'Auto-generated reports, citizen SMS updates', href: '/features/communication', color: '#0891B2' },
  { icon: '📈', title: 'Leader\'s Dashboard', desc: 'Everything on one screen, updated every 60 seconds', href: '/dashboard', color: '#BE123C' },
];

const pipelineSteps = [
  { step: '01', title: 'CITIZEN REPORTS', desc: 'Sunita sends a WhatsApp voice note in Hindi about a broken pipe at 6 AM', icon: '📱' },
  { step: '02', title: 'AI STRUCTURES', desc: 'NLP extracts: \'water pipe\', \'Ward 7\', \'leaking 3 days\'. Auto-tagged as P0 CRITICAL', icon: '🧠' },
  { step: '03', title: 'LEADER SEES', desc: 'Ward officer\'s dashboard shows this as top priority with map location & photo', icon: '👁️' },
  { step: '04', title: 'CREW DISPATCHED', desc: 'Nearest plumbing team gets GPS directions + issue brief. Arrives within 4 hours.', icon: '🚐' },
  { step: '05', title: 'VERIFIED COMPLETE', desc: 'Before/after geo-tagged photos. AI confirms repair. Sunita gets SMS confirmation.', icon: '✅' },
];
...
const rootCauses = [
  { icon: '📋', title: 'Fragmented Data', desc: 'Complaints never in one place', stat: '200+ on paper registers' },
  { icon: '⚖️', title: 'No Smart Priority', desc: 'Gas leak same queue as bench repaint', stat: 'Critical issues buried' },
  { icon: '🔍', title: 'Unverifiable Work', desc: 'Zero photo/GPS evidence', stat: '₹1.2L Cr no proof' },
  { icon: '📱', title: 'Social Media Blind Spot', desc: 'Misinformation spreads unchecked', stat: '40% scheme uptake drop' },
  { icon: '📢', title: 'Reactive Communication', desc: 'Citizens never hear back', stat: '68% no response' },
];
            ...
            {stories.map((s, i) => (
              <div key={i} className="glass-card" style={{ borderTop: `3px solid ${s.color}` }}>
                ...
                <div style={{ background: '#FEF2F2', padding: '8px 12px', borderRadius: 6, fontSize: '0.8rem', color: '#DC2626', fontWeight: 500, display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                  <span>⚠️</span><span>{s.problem}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="section" style={{ background: 'var(--bg-secondary)' }}>
        <div className="container">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0, position: 'relative' }}>
            <div style={{ position: 'absolute', left: 24, top: 0, bottom: 0, width: 3, background: 'linear-gradient(180deg, #1E40AF, #047857)', borderRadius: 2, zIndex: 0 }} />
            {pipelineSteps.map((step, i) => (
              <div key={i} style={{ display: 'flex', gap: 20, alignItems: 'flex-start', padding: '18px 0', position: 'relative', zIndex: 1 }}>
                <div style={{ width: 48, height: 48, minWidth: 48, borderRadius: '50%', background: '#1E40AF', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem', color: 'white', boxShadow: '0 4px 12px rgba(30,64,175,0.3)' }}>{step.icon}</div>
